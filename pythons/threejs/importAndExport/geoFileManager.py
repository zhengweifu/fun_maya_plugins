...
class GeoFileManager(object):
    ...
    def getFaceType(self, hasMaterial, hasFaceVertexUvs, hasFaceVertexNormals):
        faceType = 0
        faceType = self.setBit(faceType, 0, hasMaterial)
        faceType = self.setBit(faceType, 1, hasFaceVertexUvs)
        faceType = self.setBit(faceType, 2, hasFaceVertexNormals)
        return faceType
        ...
        while not polyonIterator.isDone():
            ...
            for vtx in range(polygonVertexCount):
                if vtx < polygonVertexCount - 2:
                    ...
                    face["faceType"] = self.getFaceType(hasMaterial, hasFaceVertexUvs, hasFaceVertexNormals)
                    face["vertexIndices"] = [polyonIterator.vertexIndex(0)+vertex_offset, polyonIterator.vertexIndex(vtx)+vertex_offset, polyonIterator.vertexIndex(vtx+1)+vertex_offset]
                    if hasMaterial:
                        # face.append(shaderIndex)
                        face["matIndex"] = shaderIndex
                    if hasFaceVertexUvs:
                        _index = 0
                        _uvs = []
                        for uvsetName in uvsetNames:
                            ...
                            _a = util.getInt(uv_idx)+uv_offset_list[_index]
                            polyonIterator.getUVIndex(vtx, uv_idx, uvsetName)
                            # face.append(util.getInt(uv_idx)+uv_offset_list[_index])
                            _b = util.getInt(uv_idx)+uv_offset_list[_index]
                            polyonIterator.getUVIndex(vtx+1, uv_idx, uvsetName)
                            # face.append(util.getInt(uv_idx)+uv_offset_list[_index])
                            _c = util.getInt(uv_idx)+uv_offset_list[_index]
                            _uvs.append([_a, _b, _c])
                            _index += 1
                        if _uvs:
                            face["uvIndices"] = _uvs
                    if hasFaceVertexNormals:
                        ...
                        face["normalIndices"] = [polyonIterator.normalIndex(0)+normal_offset, polyonIterator.normalIndex(vtx)+normal_offset, polyonIterator.normalIndex(vtx+1)+normal_offset]
                    # print face
                    face["hasMat"] = hasMaterial
                    face["hasNormal"] = hasFaceVertexNormals
                    face["hasUv"] = hasFaceVertexUvs
    ...
    def write(self, outMeshPath):
        ...
        try:
            ...
            for face in self.faces:
                ...
                f.write(struct.pack("B", face["faceType"]))

                for vertexIndex in face["vertexIndices"]:
                    if vertices_count > 65535:
                        f.write(struct.pack("I", vertexIndex))
                    else:
                        f.write(struct.pack("H", vertexIndex))
                if face["hasMat"]:
                    f.write(struct.pack("B", face["matIndex"]))
                if face["hasUv"]:
                    for i in range(len(face["uvIndices"])):
                        uvInd = face["uvIndices"][i]
                        if uv_counts[i] > 65535:
                            f.write(struct.pack("III", uvInd[0], uvInd[1], uvInd[2]))
                        else:
                            f.write(struct.pack("HHH", uvInd[0], uvInd[1], uvInd[2]))
                if face["hasNormal"]:
                    for normalIndex in face["normalIndices"]:
                        if normals_count > 65535:
                            f.write(struct.pack("I", normalIndex))
                        else:
                            f.write(struct.pack("H", normalIndex))            
        finally:
            f.close()
    ...
    '''ui part'''
    def ui(self):
        window_name = "GEO_FILE_MANAGER_WINDOW"
        if cmds.window(window_name, ex=True):
            cmds.deleteUI(window_name)
        
        window = cmds.window(window_name, title="Geo File Manager", widthHeight=(300, 500))
