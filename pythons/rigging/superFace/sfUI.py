...
class mainUI:
    ...
    def __init__(self,name):
        self.win = name
    def build(self):
        if windowPref(self.win,exists=1):
            windowPref(self.win,remove=1)
        ...
        window(self.win,e=1,wh=[self.win_width,self.win_height],rtf=1)
        #print 'Now size:',window(self.win,q=1,wh=1)
#        print g.SCRIPTJOB_FILE,g.SCRIPTJOB_CMD,'###########'
        cmd = 'source "' + g.SCRIPTJOB_FILE + '";' + g.SCRIPTJOB_CMD + '("'+self.win+'");';
        #print cmd
        mm.eval(cmd)
    ...
    def show(self):
        #window(self.win,e=1,w=self.win_width,h=self.win_height)
        showWindow(self.win)
...
class rootUI:
    ...
    def currentUI(self, currentRoot):
        ui = formLayout(numberOfDivisions=100)
        t = text(l='Current:',w=78,al='right')
        self.current = iconTextButton( style='textOnly',  label='' ,h=20,w=148,bgc=(0.9,1,0.9))
        self.updateCurrent(currentRoot)
        b = button(w=70,l='To Body',c=self.toBody)
        s = separator()
        formLayout(ui,e=1,
                   af=[(t,'top',5),(t,'left',0),
                       (self.current,'top',5),
                       (b,'top',5),
                       (s,'left',2),(s,'right',2)],
                   ac=[(self.current,'left',2,t),
                       (b,'left',5,self.current),
                       (s,'top',5,b)])
        '''
    ...
    def libraryUI(self, *args):
        ...
        self.libList = optionMenu('libList')#;print self.libList
        for item in list:
            menuItem(l=item)
        setParent('..')
    ...
    def toBody(self, *args):
        sel = ls(sl=1)
        if len(sel):
        	try:
                    parentConstraint(sel[0],g.FACE_ROOT_LOC,mo=1,weight=1)
                    scaleConstraint(sel[0],g.FACE_ROOT_LOC,mo=1,weight=1)
                    delete(g.ADJUSTER)
                except:
            	    pass
        else:
            print 'Select the object of body to constrain the face.'
class jointCtrlUI:
    import eyeCtrl
    import chinCtrl
    import tongueCtrl
    ...
    jointobj = jointCtrl.skinGeoASGoal()
    ...
    def build(self):
        ...
        s2 = separator()
        b3 = button(l='Convert Skin Geo To Goal',c=self.doConvertSkinGeoToGoal,vis=1)
        b4 = button(l='Paint Skin Weight Tool',c=self.weightTool,vis=1)
        formLayout(self.root,e=1,
                   af=[(b1,'top',5),(b2,'top',5),
                       (s1,'left',5),(s1,'right',5),
                       (s2,'left',5),(s2,'right',5),
                       (b3,'left',60),(b3,'right',60),
                       (b4,'left',60),(b4,'right',60)],
    ...
    def doConvertSkinGeoToGoal(self, *args):
        model = textField(self.modelUI,q=1,text=1)
        self.jointobj.convertSkinGeoToGoal(model)
    # change the state of the button
    def setConvertSkinGeoToGoalBtuttonState(self, model):
        shape = listRelatives(model, s = True)[0]
        jntObj = jointCtrlUI()
        his = listHistory(model)
        skinClusterState = 0
        for h in his:
            if nodeType(h) == 'skinCluster':
                inClusterState = 1
        bsnode = listConnections(shape, s = True, d = False, type = 'blendShape')
        if bsnode != None and inClusterState :
            pass
...
class subCtrlUI:
    ...
    def build(self):
        ...
        setParent(self.root)
        # lib
        l11 = text(l='Library:',w=78,al='right')
        ...
        s1 = separator()
        # loc add & remove
        l20 = text(l='Locator:',w=78,al='right')
        ...
        s3 = separator()
        # ctrl create & reset
        l40 = text(l='Controller:',w=78,al='right')
        ...
        self.weight = floatSliderButtonGrp( label='Weight:',
                                    ...
                                    min=0,max=10,v=1,step=0.1,
                                    bc=self.setWeight)
        l52 = button(l='Paint Skin Weight Tool',w=210,c=self.weightTool,vis=1)
        l61 = button(l='Mirror Weight +',w=100,c=self.mirrorWeightLeft)
        l62 = button(l='Mirror Weight -',w=100,c=self.mirrorWeightRight)
        formLayout(self.root,e=1,
                   af=[(l11,'top',5),
    ...
    def mirrorWeightLeft(self, *args):
        model = textField(self.modelUI,q=1,text=1)
        import mirrorCluster as mir
        obj = mir.mirrorClusterFn()
        obj.mirrorCluster(model,1)
    def mirrorWeightRight(self, *args):
        model = textField(self.modelUI,q=1,text=1)
        import mirrorCluster as mir
        obj = mir.mirrorClusterFn()
        obj.mirrorCluster(model,-1)
...
class goalCtrlUI:
    ...
    def build(self):
        ...
        lib1 = text(l='Library:',w=78,al='right')
        self.libList = optionMenu()
        for item in list:
            menuItem(l=item)
        lib3 = button(l='Load',w=50,
                      en=len(list),
                      c=self.loadLib)
        s0 = separator(hr=0,h=22)
        newlib = button(l='New',w=40,
                        c=self.showGoalCtrlLibUI)
        s1 = separator()
        b1 = button(l='Delete All Goal Models',w=210,c=self.delAGM)
        s2 = separator()
        s3 = separator()
        list1 = self.listUI()
        s4 = separator()
    ...
    def listUI(self, *args):
        ...
        t1 = text(l='Controller List:',w=78,al='right')
    ...
    def updateSetting(self, *args):
        ...
        self.attrValue = attrFieldSliderGrp(cw=[[1,60],[2,40]],pre=2,
                                            at='%s.%s'%(ctrl,attr),
                                            l=attr,
                                            #cc=self.changeLimit,
                                            min=-360,max=360,s=0.01,
                                            #fmn=-1000,fmx=1000,fs=0.01,
                                            smn=-1,smx=1,ss=0.01,
                                            )
        ...
        b5 = button(w=100,l='Mirror BS',c=self.mirrorBS)
        
        self.currentGoalListUI = textScrollList(numberOfRows=10,
                                             allowMultiSelection=0,
                                             sc=self.editGoal,
                                             dkc=self.removeGoal)
        self.updateGoalList()
        formLayout(self.settingUI,e=1,
                   af=[(self.attrValue,'top',5),(self.attrValue,'left',0),
                       (self.currentGoalListUI,'left',20),#(self.currentGoalListUI,'bottom',0),
                       (b1,'left',180),
                       (b2,'left',180),
                       (b3,'left',180),
                       (b4,'left',180),
                       (b5,'left',180)],#,(b4,'bottom',0)
                       
                   aoc=[(b5,'bottom',0,self.currentGoalListUI)],
                   ac=[(self.currentGoalListUI,'top',5,self.attrValue),
                       (self.currentGoalListUI,'right',2,b1),
                       (b1,'top',5,self.attrValue),
                       (b2,'top',5,b1),
                       (b3,'top',2,b2),
                       (b4,'top',5,b3),
                       (b5,'top',5,b4)])
    def changeLimit(self, *args):   # No need...
        v = self.__getArgs__()[2]
        n = attrFieldSliderGrp(self.attrValue,q=1,min=1)
        x = attrFieldSliderGrp(self.attrValue,q=1,max=1)
        if v>1:
            attrFieldSliderGrp(self.attrValue,e=1,max=v,min=n)
        elif v<-1:
            attrFieldSliderGrp(self.attrValue,e=1,max=x,min=v)
    def updateGoalList(self):
        textScrollList(self.currentGoalListUI,e=1,ra=1)
        args = self.__getArgs__()
        ctrl = args[0]
        attr = args[1]
        skinGeo = args[3]
        list = self.getGoalList(ctrl,attr,skinGeo)
        textScrollList(self.currentGoalListUI,e=1,append=list)
    def cleanup(self,root):
        chdn = layout(root, q=1, ca=1)
        if not chdn==None:
            for chd in chdn:
    ...
    def createNewGoal(self, *args):
        ...
        self.updateGoalList()
    def mirrorBS(self, *args):
        args = self.__getArgs__()
        drvObj = args[0]
        attr = args[1]
        drvValue = args[2]
        skinGeo = args[3]
        self.obj.mirrorConstruct(drvObj, attr, drvValue, skinGeo, 1)
    ...
    def editGoal(self, *args):
        ...
        #print ctrl,attr,value
        #print self.obj.getBlendShapeInfo(ctrl,attr,value)
        node = self.obj.getBlendShapeInfo(ctrl,attr,value,skinGeo)[4]
        #print node
        #node = node
        if node == '':
            node = self.obj.backToBS(ctrl,attr,value, skinGeo)
    ...
    def removeGoal(self, *args):
        ...
        #value = args[2]
        value = float(textScrollList(self.currentGoalListUI,q=1,si=1)[0])
        self.obj.removeBS(ctrl,attr,value, skinGeo)
        self.updateGoalList()
    ...
    def getLimitInfo(self, attr):
        pass
    def getGoalList(self, ctrl, attr, skinGeo):
        list = self.obj.getGoalList(ctrl,attr,skinGeo)
        keys = list.keys()
        try:
            keys.remove('0.0')
        except:
    ...
    def showGoalLibUI(self, *args):
        ...
        ins.build()
        ins.show()
    def delAGM(self, *args):
        if objExists(g.GOAL_MODEL_GRP):
            list = listRelatives(g.GOAL_MODEL_GRP,ad=1,type='transform',ni=1)#;print list
            if list != None:
                [delete(item) for item in list if getAttr((item+'.v'))==1]
class goalCtrlLibUI:
    win = 'goalCtrlLibUI'
    win_width = 169
    win_height = 300
